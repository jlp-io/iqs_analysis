...
def max_drawdown(log_returns):
    ...
    if isinstance(log_returns.index, pd.MultiIndex):
        ...
        log_returns.index = pd.DatetimeIndex(
            [pd.Timestamp(date(x[0], x[1], calendar.monthrange(*x)[1])) for x in log_returns.index])

    # cret_dd max value is 0 (i.e.: no drawdown)
    cret = log_returns.cumsum()
    cret_max = cret.expanding().max()
...
def risk_summary(log_returns):
    ...
    # all annualised
    out = {}
    out['Return'] = np.expm1(log_returns.mean() * _freq_count[freq])
    out['Volatility'] = annual_volatility(log_returns, freq)
    out['Sharpe'] = annual_sharpe(log_returns, freq=freq, min_ann_ret=0)
    out['Sortino'] = annual_sortino(log_returns, freq=freq, min_ann_ret=0)
